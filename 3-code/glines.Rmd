...
# calculate total emissions in CO2 eq with degassing
...
sediment_volume <- 1.51e7
sediment_volume_sd <- (2700000 *0.764555)/1.96 # conversion from cubic yards to cubic meters
...
burial_vec <- c()
for(i in 1:10000){
  temporary <- (rnorm(1, sediment_volume, sediment_volume_sd) * rnorm(1, bulk_density, bulk_density_sd) * rgamma(1, shape=a, scale = b))/(res_area * 1e6 * 83)
...
```{r sed co2 emissions}
# find exponential decay constants that lead 0.1 g/m2/yr in different number of yearsc
# plot with initial value = mean measured value
...
# }
# 
# # check this CI calculation with seth 
# res_exp_sed_co2 <- df %>% 
...
res_exp_sed_co2 <- read.csv("../2-output-data/glines_exp_sed_co2.csv")
...
eroded_vec_alt <- c()
for(i in 1:10000){
  temporary <- (rnorm(1, res_erode_g, res_erode_g_sd_alt) * rgamma(1, shape=a, scale = b) * runif(1, min = min_ox_trans, max = max_ox_trans))/(res_area * 1e6)
...
res_erode_summary_alt %>% mutate(mean_area = 1510000.0 * mean, ci_lwr_area = ci_lwr * 1510000.0, ci_upr_area = ci_upr * 1510000.0, rng = ci_upr_area - ci_lwr_area)

# reducing the sd of the g of sediment reduced the uncertainty by 1%
1-(816940320855/827901492385)

# reducing the range of % eroded by 10% reduced total uncertainty by almost 10%
1-(748695198243/827901492385)
...
integrate(function(x) as.data.frame(predict(nep_mod, newdata = data.frame(MAT = mean(temp$Temperature), Age = x, age_func = f_age(263.2, -0.19, 0, x), nutrient =as.factor("L")), interval = "prediction", level = 0.95))$fit, lower = 0, upper = 5.7)$value
```
...
write.csv(summary_df, "../2-output-data/glines_summary_nep.csv", row.names = FALSE)
```

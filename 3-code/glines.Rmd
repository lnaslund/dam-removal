...
keller_res %>% ggplot(aes(moisture_num, co2_flux))+
  geom_point()
...
df_ori <- read.csv("../1-input-data/8-besnard-data/besnard_2018.csv") %>% mutate(age_func = f_age(587.74, -0.19, -324.54, Age)) %>% mutate(nutrient = as.factor(`NA.`))
...
nep_old <- data.frame(MAT = mean(temp$Temperature), Age = seq(0, 300, by = 1), age_func = f_age(587.74, -0.19, -324.54, seq(0, 300, by = 1)), nutrient =as.factor("L"))
...
plot(0:300, y)
points(0:300, y2, col = "red")
points(0:300, y3, col = "blue")
...
plot(0:300, y)
points(0:300, y_upp, col = "red")
points(0:300, y_low, col = "blue")
...
```{r}
# nep_zero <- data.frame(MAT = mean(temp$Temperature), Age = seq(0, 10, by = 0.05), age_func = f_age(263.2, -0.19, 0, seq(0, 10, by = 0.05)), nutrient =as.factor("L"))
# nep_one <- data.frame(MAT = mean(temp$Temperature), Age = seq(0, 10, by = 1), age_func = f_age(263.2, -0.19, 0, seq(0, 10, by = 1)), nutrient =as.factor("L"))
# 
# nep_pred <- predict(nep_mod, nep_zero, interval = "prediction") %>% as.data.frame()
# nep_pred$num <- seq(0, 10, by =0.05)
# 
# nep_pred2 <- predict(nep_mod, nep_one, interval = "prediction") %>% as.data.frame()
# nep_pred2$num <- seq(0, 10, by = 1)
# 
# 
```

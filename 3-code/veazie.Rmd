...
res_eq <- res_co2_diff_eq + res_ch4_diff_eq + res_ch4_eb_eq + res_ch4_degas_eq
...
# to calculate CI in g C/m2/yr, I am assuming gases contribute to the CI in proportion to the CO2-eq contribution 
...
# summary data frame
res_emissions_summary <- data.frame(period = c("before"), 
                                    flux = c("reservoir emissions"), 
                                    variation = NA, 
                                    flux_unit = c("CO2-eq", "C"), 
                                    annual = "yes", 
                                    area = "reservoir", 
                                    mean = c(res_eq, res_g), 
                                    ci_lwr = c(res_eq_2.5, res_g_2.5), 
                                    ci_upr = c(res_eq_97.5, res_g_97.5))
```
...
veazie_sed_pred <- read.csv("../1-input-data/6-clow-data/veazie_clow.csv") %>% 
...
veazie_sed_rate_sd <- (10^(predict(sed.mod, veazie_sed_pred, interval = "prediction")[1]) - 10^(predict(sed.mod, veazie_sed_pred, interval = "prediction")[2]))/1.96
...
veazie_c_cont <- predict(c.mod, veazie_c_pred, interval = "prediction")[1] * 0.01 # convert % to proportion
veazie_c_cont_sd <- ((predict(c.mod, veazie_c_pred, interval = "prediction")[1] - predict(c.mod, veazie_c_pred, interval = "prediction")[2])/1.96)*0.01
...
for(i in 1:10000){
  ...
  temp_dbd_distr <- exp(predict(oc_dbd.mod, temp_oc_df, interval = "prediction")[1]) * 1e6 # convert g/cm3 to g/m3
  temp_dbd_distr_sd <- (exp(predict(oc_dbd.mod, temp_oc_df, interval = "prediction")[1]) * 1e6 - exp(predict(oc_dbd.mod, temp_oc_df, interval = "prediction")[2]) * 1e6)/1.96 
  ...
  temp_be_distr <- predict(be.mod, temp_be_df, interval = "prediction")[1] * 0.01 # % to proportion
...
veazie_burial <- sort(veazie_burial) # sort removes NAs, by assigning prediction interval to a normal distribution, generating some few negative values of sedimentation rate, sort here functionally truncates the distribution at 0.  
vec_length <- length(veazie_burial)

res_burial_summary <- data.frame(period = c("before"), 
                                 flux = c("reservoir burial"), 
                                 variation = NA, 
                                 flux_unit = c("CO2-eq", "C"), 
                                 annual = "yes", 
                                 area = "reservoir", 
                                 mean = c(-(veazie_burial[round(vec_length * 0.5)] * (44.01/12.01)), -veazie_burial[round(vec_length * 0.5)]), 
                                 ci_lwr = c(-(veazie_burial[round(vec_length * 0.975)] * (44.01/12.01)), -veazie_burial[round(vec_length * 0.975)]), 
                                 ci_upr = c(-(veazie_burial[round(vec_length * 0.025)] * (44.01/12.01)), -veazie_burial[round(vec_length * 0.025)]))
...
veazie_dbd_fig5 <- exp(predict(oc_dbd.mod, temp_oc_df, interval = "prediction")[1]) * 1e6

veazie_sed_fig5 <- data.frame(oc = veazie_c_cont, dbd = veazie_dbd_fig5, area = res_area)

write.csv(veazie_sed_fig5, "../2-output-data/veazie_sed_fig5.csv", row.names = FALSE)
```
...
res_sed_co2_summary <- data.frame(period = c("burp"), 
                                  flux = c("exposed sediment CO2"), 
                                  variation = res_exp_sed_co2$y %>% as.character(), 
                                  flux_unit = c(rep("CO2-eq", 6), rep("C", 6)), 
                                  annual = "no", 
                                  area = "exposed sediment area", 
                                  mean = c(res_exp_sed_co2$mean_co2, res_exp_sed_co2$mean_c), 
                                  ci_lwr = c(res_exp_sed_co2$lwr_co2, res_exp_sed_co2$lwr_c), 
...
res_sed_ch4_summary <- data.frame(period = c("burp"), 
                                  flux = c("exposed sediment CH4"), 
                                  variation = c(rep(paste0("no_pore_", thresh_0.01$y), 2), rep(paste0("pore_", thresh_0.01$y), 2)), 
                                  flux_unit = rep(c(rep("CO2-eq", 6), rep("C", 6)), 2), 
                                  annual = "no", 
                                  area = "exposed sediment area", 
                                  mean = c(thresh_0.01$mean_co2,  thresh_0.01$mean_c, thresh_0.01$mean_co2 + pore_co2_eq, thresh_0.01$mean_c + pore_c), 
                                  ci_lwr = c(thresh_0.01$lwr_co2,  thresh_0.01$lwr_c, thresh_0.01$lwr_co2 + pore_co2_eq, thresh_0.01$lwr_c + pore_c), 
                                  ci_upr = c(thresh_0.01$upr_co2, thresh_0.01$upr_c, thresh_0.01$upr_co2 + pore_co2_eq, thresh_0.01$upr_c + pore_c))
```

## Drawdown ebullition 
```{r drawdown eb}
# assume ebullition contribution to confidence interval is proportional to contribution to total CO2-eq emissions
# res_ch4_eq_eb is in g CO2-eq/m2/yr
...
res_drawdown_eb_summary <- data.frame(period = c("burp"), 
                                      flux = c("drawdown ebullition"), 
                                      variation = drawdown_eb_prop %>% as.character(), 
                                      flux_unit = c(rep("CO2-eq",5), rep("C", 5)), 
                                      annual = "no", 
                                      area = "reservoir", 
                                      mean = c(res_ch4_eb_eq * drawdown_eb_prop, res_ch4_eb_g * drawdown_eb_prop) , 
                                      ci_lwr = c(res_ch4_eb_eq_2.5 * drawdown_eb_prop, res_ch4_eb_g_2.5 * drawdown_eb_prop) , 
                                      ci_upr = c(res_ch4_eb_eq_97.5 * drawdown_eb_prop, res_ch4_eb_g_97.5 * drawdown_eb_prop))
```
...
# assuming between 0 and 100% oxidized
min_ox_trans = 0 
max_ox_trans = 1
...
for(i in 1:10000){
  ...
  temp_dbd_distr <- exp(predict(oc_dbd.mod, temp_oc_df, interval = "prediction")[1]) * 1e6 #g/cm3 -> g/m3
  temp_dbd_distr_sd <- (exp(predict(oc_dbd.mod, temp_oc_df, interval = "prediction")[1]) * 1e6 - exp(predict(oc_dbd.mod, temp_oc_df, interval = "prediction")[2]) * 1e6)/1.96 
  ...
  temp <- (runif(1, min = res_erode_g_min, max = res_erode_g_max) * temp_oc * runif(1, min = min_ox_trans, max = max_ox_trans))/(res_area) # deleted 10^6
  eroded_vec <- c(eroded_vec, temp)
}

res_erode_summary <- data.frame(period = c("burp"), 
                                flux = c("eroded sediment emissions"), 
                                variation = NA, 
                                flux_unit = c("CO2-eq", "C"), 
                                annual = "no", 
                                area = "reservoir", 
                                mean = c(eroded_vec[5000] * (44.01/12.01), eroded_vec[5000]), 
                                ci_lwr = c(eroded_vec[250] * (44.01/12.01), eroded_vec[250]), 
                                ci_upr = c(eroded_vec[9750] * (44.01/12.01), eroded_vec[9750]))
```
...
riv_co2_summary <- data.frame(period = c("build"), 
                              flux = c("river CO2 emissions"), 
                              variation = NA, 
                              flux_unit = c("CO2-eq", "C"), 
                              annual = "yes", 
                              area = "river", 
                              mean = c(riv_co2_vec[5000] * (44.01/12.01), riv_co2_vec[5000]), 
                              ci_lwr = c(riv_co2_vec[250] * (44.01/12.01), riv_co2_vec[250]), 
                              ci_upr = c(riv_co2_vec[9750] * (44.01/12.01), riv_co2_vec[9750]))
...
ch4_flux_vec_gCH4 <- ch4_flux_vec * 365 * 1e-3 * 16.04 # conversion from mmol/m2/d to g CH4/m2/yr
ch4_flux_vec_gCO2eq <- ch4_flux_vec_gCH4 * 34

print("ch4_flux_vec_gCH4 length:")
print(length(ch4_flux_vec_gCH4))

# CO2 flux
co2_vec <- sort(sample(ch4_flux_sites_usa %>% filter(is.na(CO2_Flux_Mean)==FALSE)  %>% filter(q_bin== ">100") %>% pull(CO2_Flux_Mean), 10000, replace = TRUE))* 365 * 1e-3 * 44.01
co2_vec[250]
co2_vec[5000]
co2_vec[9750]

riv_ch4_summary <- data.frame(period = c("build"), 
                              flux = c("river CH4 emissions"), 
                              variation = NA, 
                              flux_unit = c("CO2-eq", "C"), 
                              annual = "yes", 
                              area = "river", 
                              mean = c(ch4_flux_vec_gCO2eq[5000], ch4_flux_vec_gCH4[5000] * (12.01 / 16.04)), 
                              ci_lwr = c(ch4_flux_vec_gCO2eq[250], ch4_flux_vec_gCH4[250] * (12.01 / 16.04)), 
                              ci_upr = c(ch4_flux_vec_gCO2eq[9750], ch4_flux_vec_gCH4[9750] * (12.01 / 16.04)))
```
...
res_nep_summary <- data.frame(period = c("build"), 
                              flux = c("NEP"), 
                              variation = nep_df_rates$years %>% as.character(), 
                              flux_unit = c(rep("CO2-eq", 3), rep("C", 3)), 
                              annual = "yes", 
                              area = "exposed sediment area", 
                              mean = c(nep_df_rates$mean_gCO2, nep_df_rates$mean_gC), 
                              ci_lwr = c(nep_df_rates$lwr_gCO2, nep_df_rates$lwr_gC), 
                              ci_upr = c(nep_df_rates$upr_gCO2, nep_df_rates$upr_gC))
...
res_soil_ch4_summary <- data.frame(period = c("build"), 
                                   flux = c("soil CH4 emissions"), 
                                   variation = NA, 
                                   flux_unit = c("CO2-eq", "C"), 
                                   annual = "yes", 
                                   area = "exposed sediment area", 
                                   mean = c(soil_ch4_out$soil_co2_eq, soil_ch4_out$soil_c), 
                                   ci_lwr = c(soil_ch4_out$soil_co2_eq_2.5, soil_ch4_out$soil_c_2.5), 
                                   ci_upr = c(soil_ch4_out$soil_co2_eq_97.5, soil_ch4_out$soil_c_97.5))
```
...
gymno_wet_tree_ch4 <- 0.166 * 8760 * 1e-6 # ug/m2/hr -> g/m2/yr
...
summary_df <- bind_rows(res_emissions_summary, 
                        res_burial_summary, 
                        res_sed_co2_summary, 
                        res_sed_ch4_summary, 
                        res_drawdown_eb_summary, 
                        res_erode_summary, 
                        riv_co2_summary, 
                        riv_ch4_summary, 
                        res_nep_summary, 
                        ...
                        res_tree_ch4_summary) %>% 
  left_join(area_df, by = "area") %>% 
  mutate(mean_area = mean * area_m2, 
         ci_lwr_area = ci_lwr * area_m2, 
         ci_upr_area = ci_upr * area_m2) %>% 
  mutate(flux_fct = fct_relevel(flux, c("reservoir emissions", "reservoir burial", "exposed sediment CO2", "exposed sediment CH4", "drawdown ebullition", "eroded sediment emissions", "river CO2 emissions", "river CH4 emissions", "NEP", "soil CH4 emissions", "tree CH4 emissions")))


write.csv(summary_df, "../2-output-data/veazie_summary_20241213.csv", row.names = FALSE)
```
